...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $nome != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE clientes SET nome=?, cpf=?, datanasc=?, email=? WHERE id = ?");
            $stmt->bindParam(5, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO clientes (nome, cpf, datanasc, email) VALUES (?, ?, ?, ?)");
        }
    ...
    <html>
        ...
        <body>
            <form action="?act=save" method="POST" name="form1" >
               ...
               ?> />
               Data de Nascimento:
               <input type="text" name="datanasc" <?php
                ...
                try {
                    ...
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>";
                            echo "<td>".$rs->nome."</td><td>".$rs->cpf."</td><td>".$rs->datanasc."</td><td>".$rs->email
